# Demystifying the Raspberry Pi Zero 2 W Boot Process: A Bare Metal Guide
...
Think of this as the equivalent of pressing "Delete" to enter your BIOS settings. By defining parameters here, you dictate how the hardware behaves before the OS even exists. 
...
* **Reading the Map (`.dtb`):** Before the kernel can do anything useful, it reads a file called the **Device Tree Blob** (for the Zero 2 W, usually `bcm2710-rpi-zero-2-w.dtb`). This file is a literal map of the motherboard. It tells the kernel exactly which GPIO pins connect to the Wi-Fi chip and where the USB controller is.
* **Driver Activation:** Armed with the Device Tree, the kernel starts loading its internal drivers. It initializes the virtual memory, sets up the network stack, and probes the USB ports. 
...
1. **Mounting the API Filesystems:** The `init` program immediately creates the "virtual" filesystems: `/proc`, `/sys`, and `/dev`. These live in the Pi's RAM and allow your standard commands (like `fdisk` or `ps`) to see the hardware and running processes.
...
| **Config** | **4. `config.txt`** | GPU | Applies your custom hardware limits (e.g., `gpu_mem=16`). |
...
When you are building a system for digital forensics, penetration testing, or IoT, predictability is everything. By utilizing this architecture and a lightweight `init` phase instead of a heavy desktop environment, you ensure that **only** the services you explicitly want will run. There are no surprise background updates and no wasted memory—just a surgical, high-speed computing environment fully under your command.
